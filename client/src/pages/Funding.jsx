...
const fundingTypes = [
  ...
  { icon: '🦄', title: 'Angel Investors', range: '₹10L – ₹1Cr', desc: 'Warm introductions to our network of 50+ angel investors actively seeking student-led startups.', color: '#5b21b6' },
          ...
          <div className={styles.fundingGrid}>
            {fundingTypes.map((f, i) => (
              <div key={i} className={`card ${styles.fundingCard}`}>
                <div className={styles.fundIcon} style={{ background: `${f.color}15` }}><span>{f.icon}</span></div>
                <h3>{f.title}</h3>
          ...
          <div className={styles.resourcesGrid}>
            {resources.map((r, i) => (
              <div key={i} className={`card ${styles.resourceCard}`}>
                ...
                <button className="btn-outline" style={{ fontSize: 13, padding: '9px 18px', whiteSpace: 'nowrap' }}>Download</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className={styles.ctaSection}>
        <div className="container">
          <div className={styles.ctaBox}>
            ...
            <p>Apply to IgnitegenZ and get personalized fundraising support from day one.</p>
