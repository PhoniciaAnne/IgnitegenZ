...
const typeIcons = { 'Co-working Space': '🏢', 'University Lab': '🔬', 'Research Lab': '⚗️' }
        ...
        <div className="container">
          <div className={styles.searchRow}>
            <div className={styles.searchBox}>
              ...
              <input type="text" placeholder="Search by name, area, or keyword..." className={styles.searchInput} value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          ...
          {loading ? (
            <div className={styles.loading}><div className={styles.spinner}></div><p>Loading institutes...</p></div>
          ) : institutes.length === 0 ? (
            <div className={styles.empty}><p>🔎 No institutes found. Try different keywords or city.</p></div>
          ) : (
              ...
              {institutes.map(inst => (
                <div key={inst.id} className={`card ${styles.instituteCard}`}>
                  ...
                  <div className={styles.instInfo}>
                    <h3>{inst.name}</h3>
                    <p className={styles.instLocation}>📍 {inst.area}, {inst.city}</p>
                    <p className={styles.instDesc}>{inst.description}</p>
                  </div>
                  {inst.facilities?.length > 0 && (
                    <div className={styles.facilities}>
                      {inst.facilities.map(f => <span key={f} className={styles.facilityTag}>{f}</span>)}
                    </div>
                  )}
                  <div className={styles.instActions}>
                    {inst.contact_email && <a href={`mailto:${inst.contact_email}`} className="btn-primary" style={{ fontSize: 13, padding: '10px 18px' }}>Contact</a>}
                    {inst.website && inst.website !== '#' && <a href={inst.website} target="_blank" rel="noreferrer" className="btn-outline" style={{ fontSize: 13, padding: '9px 16px' }}>Visit Website</a>}
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className={styles.communitySection}>
        <div className="container">
          <div className={styles.sectionHeader}>
            ...
            <p className="section-subtitle">Connect with 500+ student entrepreneurs, co-founders, and early-stage builders across India.</p>
          </div>
