...
const categoryColors = {
  Technology: { bg: '#ede9fe', color: '#5b21b6', badge: 'badge-tech' },
        ...
        <div className="container">
          <div className={styles.filters}>
            {['All', 'Technology', 'Sustainability', 'Education'].map(cat => (
              <button key={cat} className={`${styles.filterBtn} ${category === cat ? styles.active : ''}`} onClick={() => setCategory(cat)}>
                {cat === 'All' && '🌐 '}
                {cat === 'Technology' && '💻 '}
                {cat === 'Sustainability' && '🌿 '}
                {cat === 'Education' && '📚 '}
          ...
          {loading ? (
            <div className={styles.loading}><div className={styles.spinner}></div><p>Loading mentors...</p></div>
          ) : (
                ...
                return (
                  <div key={mentor.id} className={`card ${styles.mentorCard}`}>
                    ...
                    <div className={styles.mentorActions}>
                      <a href={mentor.linkedin_url || '#'} className="btn-primary" style={{ fontSize: 13, padding: '10px 20px' }}>Connect</a>
                      <button className="btn-outline" style={{ fontSize: 13, padding: '9px 18px' }}>Book Session</button>
                    </div>
                  </div>
                )
              })}
