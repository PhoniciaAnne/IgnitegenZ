...
export default function Contact() {
  ...
  const contactInfo = [
    { icon: '📧', label: 'Email', value: 'hello@ignitegenz.in' },
    { icon: '🌐', label: 'Website', value: 'www.ignitegenz.in' },
              ...
              <div className={styles.formCard}>
                ...
                <form onSubmit={submit} className={styles.form} noValidate>
                  <div className={styles.formGroup}>
                    ...
                    <input type="text" name="name" className={`input-field ${errors.name ? styles.inputError : ''}`} placeholder="Your full name" value={form.name} onChange={handleChange} />
                    {errors.name && <span className={styles.fieldError}>{errors.name}</span>}
                  </div>
                  <div className={styles.formGroup}>
                    <label>Email Address *</label>
                    <input type="email" name="email" className={`input-field ${errors.email ? styles.inputError : ''}`} placeholder="you@example.com" value={form.email} onChange={handleChange} />
                    {errors.email && <span className={styles.fieldError}>{errors.email}</span>}
                  </div>
                  <div className={styles.formGroup}>
                    <label>Problem Statement *</label>
                    <textarea name="problem_statement" rows={4} className={`input-field ${errors.problem_statement ? styles.inputError : ''}`} placeholder="What problem are you solving? Who faces this problem?" value={form.problem_statement} onChange={handleChange} />
                    {errors.problem_statement && <span className={styles.fieldError}>{errors.problem_statement}</span>}
                  </div>
                  <div className={styles.formGroup}>
                    <label>Your Solution *</label>
                    <textarea name="solution" rows={4} className={`input-field ${errors.solution ? styles.inputError : ''}`} placeholder="How does your idea solve this problem? What makes it different?" value={form.solution} onChange={handleChange} />
